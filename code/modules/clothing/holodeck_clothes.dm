...
/obj/item/clothing/under/actorsuit/Del()
	for(var/obj/item/clothing/C in list(suit,hat,glove,shoe,mask,eye))
		derez(C)
	..()


/obj/item/clothing/under/actorsuit/proc/derez(var/obj/item/clothing/C)
	if(!C) return
	var/mob/M = C.loc
	if(istype(M))
		M.unEquip(C)
...
/obj/item/clothing/under/actorsuit/verb/clothing_interface()
...
/obj/item/clothing/under/actorsuit/verb/toggle_radio()
...
/obj/item/clothing/under/actorsuit/Hear(message, atom/movable/speaker, message_langs, raw_message, radio_freq)
	if(speaker == loc && performing)
		var/mob/living/carbon/human/H = speaker
		if(istype(H) && src == H.w_uniform)
			mic.talk_into(speaker,raw_message)


/obj/item/clothing/under/actorsuit/process()
...
/obj/item/clothing/under/actorsuit/proc/colorchange()
	var/mob/living/carbon/human/H = loc
	if(jumpsuit)
		desc = jumpsuit.desc
		name = jumpsuit.name
...
/obj/item/clothing/under/actorsuit/interact()
...
/obj/item/clothing/under/actorsuit/Topic(var/href,var/list/href_list)
	var/mob/living/carbon/human/H = usr
	if(usr != loc || !istype(H) || src != H.w_uniform) return
	if(!istype(get_area(loc),/area/holodeck))
		usr << "\red The advanced functions of this jumpsuit only work on the holodeck!"
