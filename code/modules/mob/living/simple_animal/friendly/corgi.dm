		...
		if( istype(inventory_head,/obj/item/clothing/head/helmet) && istype(inventory_back,/obj/item/clothing/suit/armor) )
			if( O.force )
				...
				for (var/mob/M in viewers(src, null))
					M.show_message("\red \b [user] hits [src] with the [O.name], however \he is too armored.")
			else
				usr << "\red [src] is wearing too much armor. You can't reach \his skin."
				for (var/mob/M in viewers(src, null))
					M.show_message("\red [user] gently taps [src] with \the [O]. ")
			if(prob(15))
...
/mob/living/simple_animal/corgi/Ian/Life()
	...
	if(!stat && !resting && !buckled)
		...
		if(prob(1))
			emote(pick("dances around","chases his tail"))
			spawn(0)
...
/mob/living/simple_animal/corgi/puppy/show_inv(mob/user as mob)
	return
