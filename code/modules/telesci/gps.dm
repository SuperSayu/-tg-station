...
/obj/item/device/gps/attack_self(mob/user)
	...
	if(emped)
		...
		for(var/obj/structure/monolith/M in global.monoliths)
			if(M.activated == 1)
				var/turf/pos = get_turf(M)
				var/area/monolith_area = get_area(M)
				var/tracked_letter = M.letter
				t += "<BR>Monolith [tracked_letter]: [z == 5 ? "Mine" : format_text(monolith_area.name)] ([pos.x], [pos.y], [pos.z])"
